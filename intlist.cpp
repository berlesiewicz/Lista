...
#include "intlist.h"
	...
	Element *IntList::create_el(int new_value)   //utworzenie nowego elementu i zwrócenie wskaŸnika
    {
        Element * new_el = new Element;
        new_el->value = new_value;
        return new_el;
    }
	int IntList::add_el_ind(int value, unsigned int index)       //dodaje element o wartoœci "value" w miejsce "index" do listy; zwraca 0 - powodzenie, -1 - niepowodzenie (za krótka lista)
    ...
    int IntList::add_el_beg(int value)       //dodanie elementu na początek
    {   if(add_el_ind(value, 0)==0) {return 0;}  //czyli dodanie na miejscu i indeksie 0
        else {return -1;}
    }
    int IntList::add_el_aft(int value, int after)        //dodaje element o wartoœci "value" po elementach o wartoœci "after" do listy; zwraca liczbê dodanych elementów
    {   Element *curr = beg;
    ...
    int IntList::del_el_ind(unsigned int index)  //usuwa el. o podanym indeksie zwr -1 jeśli się nie udało
    {   if(beg==NULL)
    ...
    int IntList::del_el_val(int valdel)   //usuwa pierwszy element o podanej wartości "valdel", zwraca indeks usuniętego elementu lub -1 jeśli nie było szukanego el.
    {   if(beg==NULL)
        {   return -1;
        }
        Element *curr = beg;
    ...
    int IntList::del_el_btw(int value_1, int value_2)    //zwraca liczbę usuniętych elementów
    {   if(value_1 > value_2)
        {   int v;
            v = value_1;
            value_1 = value_2;
    ...
    Element *IntList::get_el_ind(unsigned int index)  //zwrócenie wskaźnika na element o przekazanym indeksie (NULL jeśli nie ma takiego)
    {   Element *curr = beg;
    ...
    Element *IntList::get_el_val(int demanded)  //zwrócenie wskaźnika na element o żądanej wartości (NULL jeśli nie ma takiego)
    {   Element *curr = beg;
        while(curr!=NULL)
    ...
    int IntList::add_lists(IntList *other)    //Dodaje przekazaną listę na koniec
    {   Element *curr = beg;
        if(beg==NULL)
        {   beg = other->beg;
        }
    ...
    int IntList::subtract_lists(IntList *other)  //odejmuje od listy elementy z przekazanej listy
    ...
    IntList *IntList::copy_list()    //zwraca wskaźnik na skopiowaną listę
    ...
    int IntList::del_rep_el()    //usuwa wartości powtarzające się na liście, sostawia tyklo pierwsze elementy
    {   Element *leave = beg;
    ...
    int IntList::compare(IntList *other)     //zwraca o ile lista jest dłuższa od listy "other" (wart ujemne - "other" dłuższa, 0 - równe, wart. dodatnie - "other" krótsza
